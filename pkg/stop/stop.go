...
func Stop(id int, name, t string) {
	...
	case t == "lxc":
		...
		case name != "none":
			...
			if lxc.Data.Status == "stopped" {
				fmt.Printf("\u2705 %s Container is already stopped\n", lxc.Data.Name)
			} else {
				url := fmt.Sprintf("%s/%s/%s/status/shutdown", base_url, t, strconv.Itoa(id))
